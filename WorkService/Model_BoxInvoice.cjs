...
module.exports.UpdataStatusEdit_NotCheck = async function (req, res) {
  ...
  `;
    ...
    const result2 = await Conn.executeMany(query2, params2, {autoCommit: true, });
    res.status(200).json({ message: "Update successful",rowsAffected: result.rowsAffected,rowsAffected: result2.rowsAffected,});
    DisconnectOracleDB(Conn);
  } catch (error) {
    writeLogError(error.message);
    res.status(500).json({ message: error.message });
...
module.exports.UpdataStatusEdit_Check = async function (req, res) {
  ...
  try {
    ...
    res.status(200).json({ message: "Update successful",rowsAffected: result.rowsAffected});
    DisconnectOracleDB(Conn);
  } catch (error) {
    writeLogError(error.message);
    res.status(500).json({ message: error.message });
