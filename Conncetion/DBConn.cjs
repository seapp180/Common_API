...
const ConnectOracleDB = async (ConnType) => {
  ...
  if (ConnType === "FPC") {
    ...
    return connection;
  } else if (ConnType === "PCTT") {
    const PCTT = {
      user: process.env.PCTT_USER,
      password: process.env.PCTT_PASSWORD,
      connectString: process.env.PCTT_CONNECTION_STRING,
    };
    const connection = await oracledb.getConnection(PCTT);
    return connection;
  }
};
